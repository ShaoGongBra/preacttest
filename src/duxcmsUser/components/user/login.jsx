...
import { View, Input, Button as TaroButton } from '@tarojs/components'
import { CmsIcon } from '@/duxcms/components'
import { nav, request, toast, cmsUser, duxappTheme, useRoute, userConfig, useVerifyCode } from '@/duxcmsUser/utils'
import { Header, ScrollView, Loading, Button, Row, Text, Radio, confirm } from '@/duxui'
import './login.scss'
  ...
  return <>
    ...
    <ScrollView style={{ backgroundColor: '#fff' }}>
      <View className='cms-login'>
        <View className='cms-login__title gap-2'>
          <Text bold size={48}>您好，</Text>
          <Text bold size={48}>{bind ? '请绑定账户' : `欢迎登录${config.appName}`}</Text>
        </View>
...
export const Account = ({
  ...
  const checkAccount = useCallback(async () => {
    if ((config.phone && phoneReg.test(post.username)) || (config.email && emailReg.test(post.username))) {
